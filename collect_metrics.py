...
def main():
    ...
    parser.add_argument("--config", default="bench_config.yaml", help="Path to bench config")
    ...
    # discover instances from backend registry
    config = BenchConfig.from_yaml(args.config)
    backends = get_backends(config.backends, env)
    instances = {b.name: b.instance_id for b in backends if b.instance_id}
    ...
    for backend_name, instance_id in instances.items():
        print(f"Collecting metrics for {backend_name} ({instance_id})...")

        # CPU (built-in EC2 metric)
        cpu_points = get_metric(cw, "AWS/EC2", "CPUUtilization", instance_id,
                                start, end, args.period)
        if cpu_points:
            csv_path = out_dir / f"{backend_name}_cpu.csv"
        ...
        mem_points = get_metric(cw, "CWAgent", "mem_used_percent", instance_id,
                                start, end, args.period)
        if mem_points:
            csv_path = out_dir / f"{backend_name}_memory.csv"
